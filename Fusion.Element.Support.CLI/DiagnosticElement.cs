...
/// Connect via PuTTY (Port 9999) to view system-wide element statuses.
/// </summary>
public class DiagnosticElement : ElementBase<DiagnosticElement.State, DiagnosticElement.Event, ElementMetric>
{
    ...
    private readonly ConcurrentDictionary<string, ElementAnnouncement> _announcements = new();
    private readonly ConcurrentDictionary<string, int> _elementRows = new();
    ...
    public DiagnosticElement(IElementBlueprint config, IFireLogger logger, LoggingLevelSwitch ls, IMessageBus mb) 
    ...
    {
        if (envelope?.Payload is ElementAnnouncement announcement)
        {
            _announcements[announcement.ElementName.ToUpper()] = announcement;
            Logger.Debug("[DiagServer] Received discovery info for {Element}", announcement.ElementName);
        }
    ...
    private async Task HandleStatusMessageAsync(MessageEnvelope? message, CancellationToken ct)
    {
        ...
        string name = msg.ElementId.ElementName.ToUpper();
        if (name.Contains("MANAGER", StringComparison.OrdinalIgnoreCase)) return;

        // 1. Assign a row if new (Thread-safe assignment)
        int row = _elementRows.GetOrAdd(name, _ => Interlocked.Increment(ref _nextAvailableRow));
    ...
    private string FormatStatusLine(IElementStatus msg)
    {
        ...
        return $"{nameColor}{name,-14}{reset}{hbString}{color}{stateStr,-12}{reset}{div} {cdString} {div}{aptString}{div}{ioString}{div} {errString} {div} {resString} {div}T {displayTime}{div}{comment,-60}";
    }
    ...
    private async Task HandleNewConnection(TcpClient client)
    {
        ...
        try
        {
            ...
            if (_clients.TryAdd(clientId, writer))
            {
                ...
                string subHeader = $"\x1b[2;1H\x1b[90m{"Element CustomerName",-14} HB {"Status",-12} | Connect | PsTm ms | ↓IN /^OUT | Errors | Res:T/C/D | Time     | Comment\x1b[0m";
    ...
    private async Task ClientReadLoop(Guid id, StreamReader reader, TcpClient client)
    {
        try
        {
            ...
            while (_isRunning)
            {
                // Position prompt below the last element. Use SAVE_CURSOR so background updates restore correctly.
    ...
    private bool HandleInternalCommand(Guid clientId, string input)
    {
        ...
        switch (verb)
        {
            ...
            case "LOG":
                ...
                {
                    Reply(clientId, "Usage: LOG [ElementName] [Level]", replyRow);
                }
                return true;
            ...
            case "DESC":
                if (parts.Length >= 2)
                {
                    ...
                    ShowElementDescription(clientId, targetDev, replyRow);
                }
                else
                {
                    Reply(clientId, "Usage: DESC [ElementName]", replyRow);
                }
                return true;
            ...
            case "RESTART":
                ...
                {
                    Reply(clientId, "Usage: RESTART [ElementName]", replyRow);
                }
                return true;
            ...
            case "ONLINE":
                ...
                {
                    Reply(clientId, "Usage: ONLINE [ElementName]", replyRow);
                }
                return true;
            ...
            case "OFFLINE":
                ...
                {
                    Reply(clientId, "Usage: OFFLINE [ElementName]", replyRow);
                }
                return true;
    ...
    private async Task PublishControlCommand(string elementName, string command)
    {
        // Topic: SYS.CONTROL.ELEMENT_NAME
        var topic = new MessageBusTopic("SYS", "CONTROL", elementName);
    ...
    private void ShowElementDescription(Guid clientId, string elementName, int startRow)
    {
        if (_announcements.TryGetValue(elementName, out var ann))
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine($"--- ELEMENT DESCRIPTION: {ann.ElementName} ---");
            sb.AppendLine($"Type:     {ann.ElementType}");
        ...
        {
            Reply(clientId, $"No description available for {elementName}. Wait for discovery announcement.", startRow);
        }
    }
    ...
    private void ShowHelp(Guid clientId, int startRow)
    {
        ...
        if (page == 1)
        {
            ...
            sb.AppendLine("LOG [Element] [Level] - Set log level (e.g. LOG TPNA2 Debug)");
            sb.AppendLine("DESC [Element]        - Show element metadata & supported commands");
    ...
    public override async Task StartAsync(CancellationToken token)
    {
        ...
        RecordAppRuntime(true);
        await Machine.FireAsync(Event.Start);
    }
    public override async Task StopAsync(CancellationToken token)
    {
        Logger.Information("[{Dev}] Shutting down gracefully...", Config.Name);
        RecordAppRuntime(false);
        await Machine.FireAsync(Event.Stop);
    }

    private void RecordAppRuntime(bool isStarting)
    {
        try
        {
            // Use the established logs directory
            string logPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logs", "application_runtime.log");
            string? directory = Path.GetDirectoryName(logPath);
            if (directory != null && !Directory.Exists(directory)) Directory.CreateDirectory(directory);

            if (isStarting)
            {
                File.AppendAllText(logPath, $"[START] {DateTime.Now:yyyy-MM-dd HH:mm:ss} - Application Initialized\n");
            }
            else
            {
                var duration = DateTime.Now - _startTime;
                File.AppendAllText(logPath, $"[STOP ] {DateTime.Now:yyyy-MM-dd HH:mm:ss} - Application Shutdown | Session Runtime: {duration:hh\\:mm\\:ss}\n{new string('-', 80)}\n");
            }
        }
        catch (Exception ex)
        {
            Logger.Error(ex, "[DiagServer] Failed to record runtime log.");
        }
    }

    protected override ElementHealth MapStateToHealth(State state) => state switch
    {
        State.Running => ElementHealth.Normal,
        State.Stopping => ElementHealth.Warning,
