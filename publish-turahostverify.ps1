<#
.SYNOPSIS
    Compiles and publishes a standalone TuraHost verification tool (fbp_TuraHostVerify.json) —
    a TCP client counterpart to fbp_TuraHost.json that scripts test barcodes at a deployed
    TuraHost instance, confirms the ROUA echoes come back, and confirms the DB logging bond
    actually wrote rows for each one. Produces a deployment folder at Publish\TuraHostVerify\app.
...
.EXAMPLE
    .\publish-turahostverify.ps1
    Then, from Publish\TuraHostVerify\app: .\Fusion.Console.exe --config fbp_TuraHostVerify.json
#>
param(
...
$Customer = 'TuraHostVerify'
$BlueprintPath = Join-Path $RepoRoot 'FusionLab\ProjectConfigurations\fbp_TuraHostVerify.json'
...
# Plugins required by fbp_TuraHostVerify.json's ElementList/ReactionList.
$PluginProjects = @(
...
try {
    ...
    Write-Host "`nRun it against a deployed TuraHost by editing the IPAddress in $blueprintFileName, then:"
    Write-Host "  cd `"$AppFolder`"; .\Fusion.Console.exe --config $blueprintFileName" -ForegroundColor Yellow
}
finally {
