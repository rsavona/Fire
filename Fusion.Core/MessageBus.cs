...
public class MessageBus : IMessageBus
{
    ...
    private readonly ConcurrentDictionary<string, ConcurrentDictionary<Delegate, byte>> _patternSubscriptions;
    ...
    public MessageBus(BusAuditLogger auditLogger, ILogger logger)
    {
        ...
        _matchCache = new ConcurrentDictionary<string, List<Delegate>>(StringComparer.OrdinalIgnoreCase);
    ...
    public Task<bool> SubscribeAsync(string topic, Delegate handler)
    {
        ...
        bool isPattern = topic.Contains('*') || topic.Contains('#');
        var dict = isPattern ? _patternSubscriptions : _subscriptions;
        
        var handlers = dict.GetOrAdd(topic, _ => new ConcurrentDictionary<Delegate, byte>());
        bool added = handlers.TryAdd(handler, 0);
        
        if (added)
        {
            if (isPattern) _matchCache.Clear(); // Invalidate cache on pattern changes
    ...
    public void Unsubscribe(string topic, Delegate handler)
    {
        ...
        if (dict.TryGetValue(topic, out var handlers))
        {
            if (handlers.TryRemove(handler, out _))
            {
                ...
                if (handlers.IsEmpty) dict.TryRemove(topic, out _);
            }
        }
    }
    ...
    private async Task DispatchMessageInternal(string topic, MessageEnvelope envelope, CancellationToken token = default)
    {
        ...
        // 1. Global Subscribers (Always execute)
        ...
        // 2. Exact + Pattern Subscribers (The Fast Path)
        var handlersToInvoke = GetHandlersForTopic(topic);
        ...
        foreach (var handler in handlersToInvoke)
        {
            ...
            string handlerName = isVerbose ? $"{handler.Method.DeclaringType?.Name}.{handler.Method.Name}" : string.Empty;
    ...
    /// <summary>
    /// Gets all handlers (exact + pattern matches) for a given topic.
    /// This is the fast-path engine.
    /// </summary>
    private List<Delegate> GetHandlersForTopic(string topic)
    {
        // 1. Check the match cache (O(1) fast path)
        if (_matchCache.TryGetValue(topic, out var cachedHandlers)) return cachedHandlers;

        // 2. Compute the matches
        var result = new List<Delegate>();

        // Add exact matches
        if (_subscriptions.TryGetValue(topic, out var exactHandlers))
        {
            result.AddRange(exactHandlers.Keys);
        }

        // Add pattern matches
        if (!_patternSubscriptions.IsEmpty)
        {
            foreach (var pattern in _patternSubscriptions)
            {
                if (IsTopicMatch(topic, pattern.Key))
                {
                    result.AddRange(pattern.Value.Keys);
                }
            }
        }
    ...
    /// <summary>
    /// Helper to check if a concrete topic matches a pattern.
    /// '*' matches one segment, '#' matches all remaining segments.
    /// </summary>
    private static bool IsTopicMatch(string topic, string pattern)
    {
        ...
        for (int i = 0; i < patternParts.Length; i++)
        {
            ...
            if (p == "#") return true; // Multi-level wildcard matches everything remaining

            if (i >= topicParts.Length) return false; // Topic shorter than pattern

            if (p == "*") continue; // Single-level wildcard matches any segment
