...
public class NatsDevice : ClientDeviceBase, IMessageProvider
{
    ...
    public NatsDevice(IMessageBus bus, IDeviceConfig config, IFireLogger deviceLogger, LoggingLevelSwitch ls)
        : base(bus, config, deviceLogger, ls, true)
    {
        _url = ConfigurationLoader.GetOptionalConfig(config.Properties, "Url", "nats://127.0.0.1:4222");
        _heartbeatSubject = $"{Key.DeviceName}.heartbeat";
        
        Logger.Information("[{Dev}] Initializing NATS Device. URL: {Url}", Config.Name, _url);
    }
    ...
    public override async Task SendAsync(string message, CancellationToken token, bool fireEvent = true)
    {
        ...
        var subject = ConfigurationLoader.GetOptionalConfig(Config.Properties, "DefaultWriteSubject", $"{Key.DeviceName}.out");
