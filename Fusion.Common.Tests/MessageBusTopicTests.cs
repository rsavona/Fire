...
public class MessageBusTopicTests
{
    ...
    [InlineData(".", "", "", "")]
    public void Constructor_WithStrTopic_ParsesCorrectly(string topic, string expectedElement, string expectedType, string expectedDiscriminator)
    {
        // Act
        var result = new MessageBusTopic(topic);

        // Assert
        Assert.Equal(expectedElement, result.ElementName);
