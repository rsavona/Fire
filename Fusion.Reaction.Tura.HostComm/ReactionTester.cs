...
public class ReactionTester : ReactionBase
{
    ...
    public ReactionTester(IMessageBus bus, IReactionBlueprint config, ILogger logger)
        : base(bus, config, logger)
    {
        _targetElement = config.Properties.TryGetValue("TargetElement", out var d) ? d.ToString() ?? "" : "";
        _intervalMs = config.Properties.TryGetValue("IntervalMs", out var i) ? Convert.ToInt32(i) : 5000;
        
        Logger.Information("[{Reaction}] Tura Tester (Simulating Host & Scanner) Initialized. Target: {Target}", 
            Config.Name, _targetElement);

        MessageBus.SubscribeAsync(MessageBusTopic.ElementStatus.ToString(), HandleElementStatusAsync);
    }

    private async Task HandleElementStatusAsync(MessageEnvelope? envelope, CancellationToken ct)
    {
        if (envelope?.Payload is IElementStatus status && 
            status.ElementId.ElementName == _targetElement && 
            status.State.Equals("Connected", StringComparison.OrdinalIgnoreCase))
        {
            Logger.Information("[{Reaction}] Host Simulator CONNECTED. Starting simulation.", Config.Name);
        }
    }
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (IsSystemStarted)
                {
                    ...
                    Logger.Information("[{Reaction}] SIMULATING SCAN: {Barcode}", Config.Name, barcode);
                    ...
                    if (_messageIndex % 3 == 0)
                    {
                        ...
                        var tstTopic = $"{_targetElement}.Outbound.Test";
    ...
    public async Task<object?> HandleHostReceive(MessageEnvelope envelope, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (msgType == "CSCN")
            {
                ...
                Logger.Information("[{Reaction}] HOST SIMULATOR: Received CSCN for {Barcode}. Sending log-formatted ROUT.", Config.Name, barcode);
            ...
            if (msgType == "ROUA")
            {
                Logger.Information("[{Reaction}] HOST SIMULATOR: Received ROUA (Echo) from Fire.", Config.Name);
            }
        }
        catch (Exception ex)
        {
