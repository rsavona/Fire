...
public class ReactionTester : ReactionBase
{
    ...
    public ReactionTester(IMessageBus bus, WorkflowConfig config, ILogger logger)
    ...
    {
        if (envelope?.Payload is ElementStatusMessage status && 
            status.ElementId.DeviceName == _targetDevice && 
