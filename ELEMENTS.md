...
**Wire protocol:** newline-terminated compact JSON frames (`Subscribe`, `Unsubscribe`, `Publish`, `Heartbeat`, `HeartbeatAck`, plus `SubscribeQueue`/`UnsubscribeQueue`/`QueuePublish`/`Ack` for competing-consumer queue groups). Envelopes republished from a remote instance carry a `LINK:<origin>` client tag, which acts as a loop guard: anything that arrived over a link is never forwarded back out, preventing echo storms when two instances link to each other. Clients joining the same `QueueGroup` on a topic each receive an exclusive, acked, round-robin share of the traffic (at-least-once, redelivered on timeout/disconnect) instead of a fan-out copy.

**Key properties:** `IPAddress`/`Port` (client), `Port`/`MaxClients` (server), `Origin` (instance identity), `RemoteTopics`, `PublishTopics`, `QueueTopics` (semicolon-separated patterns), `QueueGroup` (competing-consumer group, default `Origin`), `HeartbeatIntervalMs`, `AckTimeoutMs`/`RedeliverAttempts` (server queue redelivery). See `fbp_LinkDemoServer.json` / `fbp_LinkDemoClient.json` for a working pair, and `Fusion.Element.Link/_documentation/ReadMe-Link.md` for the full protocol, multi-connection topology, and configuration reference.
