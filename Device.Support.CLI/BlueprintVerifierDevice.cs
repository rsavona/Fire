...
public class BlueprintVerifierDevice : DeviceBase<BlueprintVerifierDevice.State, BlueprintVerifierDevice.Event, BlueprintVerifierDevice.VerifierMetric>
{
    ...
    public BlueprintVerifierDevice(IMessageBus bus, IDeviceConfig config, IFireLogger logger) 
    ...
    public override async Task StartAsync(CancellationToken token)
    {
        ...
        UpdateStatus(State.Monitoring, Event.Start, DeviceHealth.Normal, "Verifier Active");

        // 1. Monitor Element Health
        await MessageBus.SubscribeAsync(MessageBusTopic.DeviceStatus.ToString(), async (envelope, ct) =>
        {
            if (envelope.Payload is DeviceStatusMessage msg && msg.Health is DeviceHealth.Warning or DeviceHealth.Critical)
            {
                await LogBugAsync("Health Degradation", $"Element '{msg.DeviceId.DeviceName}' reported {msg.Health} state. Comment: {msg.Comment}");
            }
        });
    ...
    private async Task MonitorLoopAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                _ = ConfigurationLoader.UpdateDevicePropertyAsync("SYSTEM", "SimulationRuntime", runtime);
            }
    ...
    private async Task LogBugAsync(string type, string description)
    {
        ...
        UpdateStatus(State.BugDetected, Event.ErrorFound, DeviceHealth.Warning, $"Last Bug: {type}");
        
        // Reset Stability timer in blueprint
        _ = ConfigurationLoader.UpdateDevicePropertyAsync("SYSTEM", "StabilityDuration", TimeSpan.Zero);
    ...
    protected override DeviceHealth MapStateToHealth(State state) => state switch
    {
        ...
        State.BugDetected => DeviceHealth.Warning,
