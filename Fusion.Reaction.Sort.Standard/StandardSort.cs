...
public class StandardSort : ReactionBase
{
    ...
    public StandardSort(IMessageBus bus, ReactionBlueprint config, ILogger logger)
        : base(bus, config, logger)
    {
        ...
        Logger.Information("[{Reaction}] Initialized with {Count} exits: {Exits}", 
    ...
    public async Task<object?> HandleInductionRoundRobinSort(MessageEnvelope envelope, CancellationToken ct)
    {
        if (_diverts.Count == 0)
        {
            Logger.Warning("[{Reaction}] No Diverts configured for RoundRobinSort.", Config.Name);
            return null;
        }
        ...
        try
        {
            ...
            if (dp == null || gin == null)
            {
                Logger.Warning("[{Reaction}] Invalid payload for RoundRobinSort: Missing DecisionPoint or GIN.", Config.Name);
                return null;
            }
            ...
            Logger.Information("[{Reaction}] RoundRobin: GIN {Gin} at {DP} -> Selected Exit: {Exit}", 
        ...
        {
            Logger.Error(ex, "[{Reaction}] Error in RoundRobinSort for GIN {Gin}", Config.Name, envelope.Gin);
            return null;
        }
    }
    ...
    public async Task<object?> HandleInductionSqlSort(MessageEnvelope envelope, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (dp == null || gin == null)
            {
                Logger.Warning("[{Reaction}] Invalid payload for SqlSort: Missing DecisionPoint or GIN. Payload: {Payload}", Config.Name, payloadStr);
                return null;
            }

            string spName = "[usp_GetSorterAssignment]";

            Logger.Information("[{Reaction}] SQL Sort SP Request: {SP} for Barcode: {BC}, GIN: {Gin}", 
        ...
        {
            Logger.Error(ex, "[{Reaction}] Error in HandleInductionSqlSort for GIN {Gin}. Payload: {Payload}", Config.Name, envelope.Gin, payloadStr);
            return null;
        }
    }
    ...
    public async Task<object?> HandleDestinationResult(MessageEnvelope envelope, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (dp == null || gin == null)
            {
                Logger.Warning("[{Reaction}] Invalid Database response: Missing DecisionPoint or GIN. Payload: {Payload}", Config.Name, payloadStr);
                return null;
            }
            ...
            if (results != null && results.Count > 0)
            {
                foreach (var rowNode in results)
                {
                    ...
                    {
                        Logger.Warning("[{Reaction}] Unexpected result format in SQL row: {Kind}. Row Data: {Data}", 
                    ...
                    if (string.IsNullOrWhiteSpace(exit) && keys.Count > 0)
                    {
                        ...
                        if (firstNode != null)
                        {
                            ...
                            Logger.Information("[{Reaction}] No named column matched. Falling back to first column '{Key}': {Value}", 
                    ...
                    {
                        Logger.Warning("[{Reaction}] SQL Row found but could not extract exit. GIN: {Gin}, Available Keys: {Keys}, Row: {Row}", 
            ...
            if (actions.Count == 0)
            {
                Logger.Warning("[{Reaction}] No exits found in DB for GIN {Gin}. Results Count: {Count}. Payload: {Payload}", 
                    Config.Name, gin, results?.Count ?? 0, payloadStr);
                actions.Add("15");
            }

            Logger.Information("[{Reaction}] SQL Async Result: GIN {Gin} at {DP} -> Actions: {Actions}", 
        ...
        {
            Logger.Error(ex, "[{Reaction}] Error in HandleDestinationResult. Payload: {Payload}", Config.Name, payloadStr);
            return null;
        }
    }
}
