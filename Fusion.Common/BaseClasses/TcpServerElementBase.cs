...
public abstract class TcpServerElementBase<TProcessor>
    : ElementBase<TcpServerElementBase<TProcessor>.State, TcpServerElementBase<TProcessor>.Event, ElementMetric>
    where TProcessor : IMessageProcessor 
{
    ...
    protected TcpServerElementBase(IMessageBus bus, IElementBlueprint config, IFireLogger logger, TProcessor processor,
        ...
        : base(bus, config, logger, swtch, State.Offline, Event.Start)
    {
        ...
        int maxBufferSize = config.Properties.TryGetValue("MaxBufferSize", out var bufSize)
    ...
    protected sealed override void ConfigureStateMachine()
    {
        ...
        Machine.Configure(State.Listening)
            ...
            .Ignore(Event.ServerStarted) // Ignore if server restarts while already listening/connected
        ...
        Machine.Configure(State.Faulted)
            ...
            .Ignore(Event.ClientConnected)
            .InternalTransition(Event.ClientDisconnected, transition =>
            {
                if (transition.Parameters.Length > 0 && transition.Parameters[0] is string clientId)
                {
    ...
    /// <summary>
    /// Orchestrates the element startup sequence.
    /// </summary>
    ...
    /// <summary>
    /// Orchestrates the element shutdown sequence.
    /// </summary>
    ...
    {
        Logger.Error("[{Dev}] Element entered FAULTED state. Stopping server.", Config.Name);
        _ = Server.StopAsync();
    }
    ...
    {
        State.Connected or State.Processing => ElementHealth.Normal,
        State.Listening => ElementHealth.Warning, // Warning because no client is active
        State.Faulted => ElementHealth.Critical,
        State.Offline or State.Stopping => ElementHealth.Warning,
