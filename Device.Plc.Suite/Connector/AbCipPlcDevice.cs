...
public class AbCipPlcDevice : ClientDeviceBase
{
    ...
    public AbCipPlcDevice(IMessageBus bus, IDeviceConfig config, IFireLogger logger, LoggingLevelSwitch ls)
    ...
    protected override Task DeviceDisconnectedAsync()
    {
        _pollingCts?.Cancel();
        _requestTag?.Dispose();
        _responseTag?.Dispose();
        _updateTag?.Dispose();
        return base.DeviceDisconnectedAsync();
    }
    ...
    {
        Logger.Warning("[{Dev}] CIP Device Faulted.", Config.Name);
    }
}
