...
public class PlcDeviceManager : DeviceManagerBase<PlcServerDevice>
{
    ...
    protected override void SubscribeToRouteDestinations(IDevice device)
    {
        if (device is PlcServerDevice dev)
        {
            ...
            {
                var routes = w.Routes.Where(r => r.Destination?.StartsWith(device.Config.Name) == true);
                foreach (var r in routes)
                {
                    // 1. Use a lambda (msg) => ... to pass a delegate
                    // 2. Specify the message type (e.g., <string>) to help the compiler
                      MessageBus.SubscribeAsync(r.Destination, async (MessageEnvelope msg) =>
                    {
                    ...
                    _expectResponseTopics.Add(r.Source);
                }
            }
        }
    }
