...
public class PlcServerElement : TcpServerElementBase<PlcMessageProcessor>, IMessageProvider
{
    ...
    /// <summary>
    /// Represents a multi-client element implementation for connecting to PLCs (Programmable Logic Controllers).
    ...
    /// </summary>
    /// <remarks>
    /// The implementation is based on the TcpServerMultiClientElementBase class, providing core support for client-management
    ...
    /// </remarks>
    /// <param name="config">
    /// An instance of IElementBlueprint providing configuration for the element, including settings such as element properties.
    /// </param>
    /// <param name="elementLogger">
    /// An instance of ILogger used for logging messages and diagnostic information for the element.
    /// </param>
    public PlcServerElement(IMessageBus bus, IElementBlueprint config, IFireLogger elementLogger, LoggingLevelSwitch ls)
        : base(
            bus,
            config,
            elementLogger,
            new PlcMessageProcessor(new PlcMessageParser(), config.Name, elementLogger), ls,
            ConfigurationLoader.GetRequiredConfig<int>(config.Properties, "ElementPort"),
            terminalStr: new DelimiterSetStrategy(new byte[] { 0x03 })
            , 99)
    {
        _ginLearner = new GinSequenceLearner(config.Name, elementLogger);
        ...
        LogControl.SetElementLevel(Key.ElementName, LogEventLevel.Verbose);
    }
    ...
    /// updates the element status tracker, notifies clients about the status change,
    ...
    public async Task<bool> SendResponseAsync(object jsonPayload, string clientKey)
    {
        ...
        {
            finalPayload = PlcMessageParser.FrameResponse(jsonPayload, Key.ElementName);
        }

        var ret = await Server.SendResponseAsync(Key.ElementName, clientKey, finalPayload);
        if (ret)
        {
    ...
    /// <summary>
    /// Handles the heartbeat received event from the processor, updating the element status tracker,
    /// </summary>
